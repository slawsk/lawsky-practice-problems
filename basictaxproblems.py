...
def rates_problems(type_problem='random'):
    ...
    married = fm.RatesProb(f"{person1.name} and {person2.name} are married and file jointly.",'their',fm.married)
    single = fm.RatesProb(f'{person1.name} is single and files as a single person.',person1.poss,fm.single)
    hoh = fm.RatesProb(f'{person1.name} is single and files as a head of household.',person1.poss,fm.hoh)
    ...
    if bracket_choice == df.index.max():
        explanation_brackets = f"{status.pronouns} taxable income is above {fm.ac(bottom_bracket)}"
    else:
    ...
    if type_problem == 'marginal':
        ...
        if bracket_choice == 0:
            ...
            judgements[tax_on_marginal_rate]='This would be the correct answer if the marginal rate were applied to all of the earnings of the taxpayer.'
            for item in tax_owed_list:
                if item != correct:
                    possibleanswers.append(item)
                    judgements[item]="What type of taxpayer is this (single, married, head of household)?"
...
def tvm():
    ...
    correct = -int(relevant_function(initial_rate/100,length_time,0,value,when='end'))
    ...
    monthly_compounding = -int(relevant_function(initial_rate/1200,length_time*12,0,value,when='end'))
    
    semiannual_compounding = -int(relevant_function(initial_rate/200,length_time*2,0,value,when='end'))
    
    monthly_compounding_correct_periods = -int(relevant_function(initial_rate/1200,length_time,0,value,when='end'))
    
    semiannual_compounding_correct_periods = -int(relevant_function(initial_rate/200,length_time,0,value,when='end'))
    
    possibleanswers = [correct,monthly_compounding, semiannual_compounding,monthly_compounding_correct_periods,semiannual_compounding_correct_periods]
    
    judgements = {correct:explanation,monthly_compounding:'This would be the correct answer for monthly compounding (i.e., compounding 12 times per period).',semiannual_compounding:'This would be the correct answer for semiannual compounding (i.e., compounding 2 times per period).',monthly_compounding_correct_periods:f'This would be the correct answer for {length_time} months, with an annual rate of {initial_rate}%, and thus a monthly rate of 1/12 of that.',semiannual_compounding_correct_periods:f'This would be the correct answer for {length_time} six-month periods, with an annual rate of {initial_rate}%, and thus a semiannual rate of 1/2 of that.'}    
...
def bonds_COD(problem_type='random'):
    ...
    present_value = npf.pv(future_rate/100,time-time_passed,pmt,future_value,when='end')
    ...
    random_no_lossnumber = int(abs((no_lossnumber - round(.8*random.random()*no_lossnumber))))
    ...
    if problem == problem_166:
        ...
        full_amount_ordinary = f'{fm.ac(future_value)} of ordinary loss'

        full_amount_capital = f'{fm.ac(future_value)} of capital loss'
        
        possibleanswers = [zero,correct_num_ordinary,correct_num_capital,no_loss_ordinary,no_loss_capital,full_amount_ordinary,full_amount_capital]
        
        judgements =  {correct_num_ordinary:f'<p>Correct! When the debt was cancelled by the initial lender, there was an ordinary (bad debt) loss, under <a href="https://www.law.cornell.edu/uscode/text/26/166" target="_blank" rel="noreferrer">Section 166</a>, in the amount that went unpaid.</p>',
...
def COD_simple():
    ...
    judgements = {correct:f'{person.name} has income to the extent of the cancelled debt, that is, the remaining debt after the amount he paid off. {fm.ac(loan)} - {fm.ac(paid_off)} = {fm.ac(forgiven)}. Section 61(a)(11).'}
...
def satisfy_debt():
    ...
    basis = pays_back - fm.nearesthundred(random.randint(1,round(.8*pays_back)))
   
    #how does she pay it back?
    cash_paid = SatisfyAnswer(f'paying cash of {fm.ac(pays_back)}',0,'','')
    services_performed = SatisfyAnswer(f'performing services worth {fm.ac(pays_back)}',pays_back,f'In addition, {person.nom} {correcthas} compensation income due to using services to satisfy the debt.','What about the provision of services? There should not be an incentive to satisfy debt with services.')
    asset_transferred = SatisfyAnswer(f'transferring an asset worth {fm.ac(pays_back)} with a basis of {fm.ac(basis)}',pays_back-basis,f'In addition, {person.nom} {correcthas} gain due to the disposition of the asset equal to the value of the asset, {fm.ac(pays_back)}, minus its basis, {fm.ac(basis)}, which equals {fm.ac(pays_back-basis)}.','What about the disposition of the asset? There should not be an incentive to satisfy debt with appreciated assets.')
    ...
    judgements =  {correct:f'That is correct. {person.name} has cancellation of indebtedness income equal to the difference between the amount {person.nom} owed, {fm.ac(borrowing)}, and the amount {person.nom} paid in full satisfaction of the loan, {fm.ac(pays_back)}--that is, {fm.ac(COD_income)}. {extra_correct}',
...
def restricted_property(type_problem='random'):
    ...
    if forfeit:
        ...
        addl_questions = [question_include_sale,question_corp_deduct_vest,question_include_vest]

    possible_questions = [question_corp_deduct_grant,question_include_grant]+addl_questions
    ...
    if type_problem == 'random':
        ...
        if forfeit:
            ...
            question_lang = random.choice([question_corp_deduct_grant,question_corp_deduct_vest])
    elif type_problem == 'employee':
        if forfeit:
            question_lang = random.choice([question_include_grant,question_deduct_forfeit])
        else:
            question_lang = random.choice([question_include_grant,question_include_vest,question_include_sale])
    elif type_problem == 'forfeit':
        question_lang = random.choice([question_deduct_forfeit,question_include_forfeit])
    ...
    if election_83 and forfeit:
        ...
        if question_lang == question_deduct_forfeit:
            possibleanswers = [f"{fm.ac(0)}",value_at_grant_ordinary,value_at_grant_capital,amount_paid_at_grant_ordinary,amount_paid_at_grant_capital,value_at_forfeit_ordinary,value_at_forfeit_capital]
            
        else:
             possibleanswers = [0,value_at_grant,amount_paid_at_grant,value_at_grant-amount_paid_at_grant,value_at_forfeit,value_at_forfeit-amount_paid_at_grant,value_at_sale - value_at_vest]

        if question_lang == question_corp_deduct_grant or question_lang == question_include_grant:

            
            correct = value_at_grant - amount_paid_at_grant
            judgements = {correct:'<p>That is correct. Because there was an election under <a href="https://www.law.cornell.edu/uscode/text/26/83" target="_new" rel="noreferrer">Section 83(b)</a>, the employee includes, and the employer deducts, the difference between the value at grant and the amount paid at grant.</p>',
        ...
        if question_lang == question_deduct_forfeit:
            ...
            judgements = {correct:f'<p>That is correct. Because {person.name} made an election under <a href="https://www.law.cornell.edu/uscode/text/26/83" target="_new" rel="noreferrer">Section 83(b)</a> and the stock is a capital asset in {person.poss} hands, <a href="https://www.law.cornell.edu/cfr/text/26/1.83-2" target="_new" rel="noreferrer">Treas. Reg. 1.83-2(a)</a> permits a capital loss equal to the amount paid for the stock, reduced by the amount {person.nom} received on forfeit. In this case, because {person.nom} received nothing on forfeit, the capital loss equals {fm.ac(amount_paid_at_grant)} per share.</p>',
            ...
            judgements[value_at_forfeit_ordinary]=judgements[value_at_forfeit_capital]=judgements[value_at_grant_capital]=judgements[value_at_grant_ordinary]='<p>Consider <a href="https://www.law.cornell.edu/cfr/text/26/1.83-2" target="_new">Treas. Reg. 1.83-2(a)</a>.</p>'
                         

        if question_lang == question_include_forfeit:
            correct = value_at_grant-amount_paid_at_grant
            judgements = {correct:'That is correct. The corporation had the benefit of a deduction, and must unwind that benefit when it gets the property back.',
    ...
    if election_83 and not forfeit:
        possibleanswers = [0,value_at_grant,amount_paid_at_grant,value_at_grant-amount_paid_at_grant,value_at_vest,value_at_sale,value_at_vest-amount_paid_at_grant,value_at_sale-amount_paid_at_grant,value_at_sale-value_at_grant,value_at_sale - value_at_vest]
        ...
        if question_lang == question_corp_deduct_vest or question_lang == question_include_vest:
            correct = 0
            judgements = {correct:'<p>That is correct. Because there was an election under <a href="https://www.law.cornell.edu/uscode/text/26/83" target="_new" rel="noreferrer">Section 83</a>, the compensation event occurred at grant, and there is no income for the employee, or associated deduction for the employer, when the property vests.</p>',
                    value_at_vest-amount_paid_at_grant:'<p>Remember: there was a <a href="https://www.law.cornell.edu/uscode/text/26/83" target="_new" rel="noreferrer">Section 83(b)</a> election.</p>',
        ...
        if question_lang == question_include_sale:
            ...
            judgements = {correct:f'That is correct. The amount included in the sale is the gain from the sale of the stock, that is, the difference between the amount received, {fm.ac(value_at_sale)}, and the basis of the stock. The basis of the stock equals the amount paid for the stock--in this case, {fm.ac(amount_paid_at_grant)}--plus the amount previously included in gross income with respect to the stock--in this case, {fm.ac(value_at_grant-amount_paid_at_grant)}.',
                          value_at_sale:'What about the basis of the stock?',
                          value_at_sale-value_at_vest:'<p>What about the election under <a href="https://www.law.cornell.edu/uscode/text/26/83" target="_new" rel="noreferrer">Section 83(b)</a>?</p>'
                    }


    if (not election_83) and forfeit:
        if question_lang == question_deduct_forfeit:
            possibleanswers = [0,value_at_grant_ordinary,value_at_grant_capital,amount_paid_at_grant_ordinary,amount_paid_at_grant_capital,value_at_forfeit_ordinary,value_at_forfeit_capital]
        ...
        if question_lang == question_corp_deduct_grant or question_include_grant:
            ...
            correct = 0
            judgements = {correct:'<p>That is correct. Because there was no election under <a href="https://www.law.cornell.edu/uscode/text/26/83" target="_new" rel="noreferrer">Section 83(b)</a>, there is no income for the employee under <a href="https://www.law.cornell.edu/uscode/text/26/83" target="_new" rel="noreferrer">83(a)</a>, or associated deduction for the employer under <a href="https://www.law.cornell.edu/uscode/text/26/83" target="_new" rel="noreferrer">83(h)</a>, until the property vests.</p>',
                          value_at_grant-amount_paid_at_grant:'<p>Remember: there was no <a href="https://www.law.cornell.edu/uscode/text/26/83" target="_new" rel="noreferrer">Section 83(b)</a> election. Is the property subject to substantial risk of forfeiture and nontransferable?</p>',
        ...
        if question_lang == question_deduct_forfeit:
            ...
            judgements = {correct:f'<p>That is correct. Because {person.name} made no elections with respect to the stock, <a href="https://www.law.cornell.edu/cfr/text/26/1.83-1" target="_new" rel="noreferrer">Treas. Reg. 1.83-1(b)(2)</a> permits an ordinary deduction for the amount paid for the stock reduced by the amount received on forfeiture. In this case, because {person.name} received nothing on forfeiture, {person.nom} may take an ordinary deduction of {fm.ac(amount_paid_at_grant)} per share.</p>',
            ...
            judgements[value_at_forfeit_ordinary]=judgements[value_at_forfeit_capital]=judgements[value_at_grant_capital]=judgements[value_at_grant_ordinary]='<p>Consider <a href="https://www.law.cornell.edu/cfr/text/26/1.83-2" target="_new">Treas. Reg. 1.83-1(b)(2)</a>.</p>'
                        
        elif question_lang == question_include_forfeit:
            correct = 0
            judgements = {correct:f'That is correct. {employer} has taken no deduction with respect to this transfer, and thus has no income when the stock is returned.',
    ...
    if not election_83 and not forfeit:
        ...
        if question_lang == question_corp_deduct_vest or question_lang == question_include_vest:
            ...
            judgements = {correct:'<p>That is correct. Because there was no <a href="https://www.law.cornell.edu/uscode/text/26/83" target="_new" rel="noreferrer">Section 83(b)</a> election, income for the employee under <a href="https://www.law.cornell.edu/uscode/text/26/83" target="_new" rel="noreferrer">83(a)</a>, and the associated deduction for the employer under <a href="https://www.law.cornell.edu/uscode/text/26/83" target="_new" rel="noreferrer">83(h)</a>, occur when the property vests.</p>',
                          value_at_grant-amount_paid_at_grant:'<p>Remember: there was no <a href="https://www.law.cornell.edu/uscode/text/26/83" target="_new" rel="noreferrer">Section 83(b)</a> election. Is the property subject to substantial risk of forfeiture and nontransferable?</p>',
        ...
        if question_lang == question_include_sale:
            ...
            judgements = {correct:f'That is correct. The amount included in income due to the sale is the gain from the sale of the stock, that is, the difference between the amount received {fm.ac(value_at_sale)} and the basis of the stock. The basis of the stock equals the amount paid for the stock--in this case, {fm.ac(amount_paid_at_grant)}--plus the amount previously included in gross income with respect to the stock--in this case, {fm.ac(value_at_vest-amount_paid_at_grant)}.', value_at_sale-value_at_grant:'<p>Remember: there was no <a href="https://www.law.cornell.edu/uscode/text/26/83" target="_new" rel="noreferrer">Section 83(b)</a> election.</p>',value_at_vest:'What about the amount paid for the stock?'
                          }
...
def options_as_comp():
    ...
    possibleanswers = [0,value_at_grant*number_of_shares,value_at_exercise*number_of_shares,(value_at_exercise-strike_price)*number_of_shares,value_at_sale*number_of_shares,(value_at_sale - strike_price)*number_of_shares,(value_at_sale-value_at_exercise)*number_of_shares,(value_at_sale-value_at_grant)*number_of_shares,value_of_option,(value_at_sale - strike_price)*number_of_shares-value_of_option]

    FMV_nonzero = [question_corp_deduct_grant,question_include_grant]
    no_FMV_nonzero = [question_corp_deduct_exercise,question_include_exercise,question_include_sale]
    ...
    if readily_asc_FMV == 'no':
        ...
        if question_lang == question_corp_deduct_grant or question_lang == question_include_grant:
            correct = 0
            judgements = {correct:f'<p>That is correct. Because the option does not have a readily ascertainable fair market value, there is no income for the employee, or associated deduction for the employer, until the option is exercised, under <a href="https://www.law.cornell.edu/uscode/text/26/83" target="_new" rel="noreferrer">Section 83(e)(3)</a>.</p>',
                          value_at_grant*number_of_shares:'Remember: this is an option. Additionally, the option itself has no readily ascertainable fair market value.'
                          }

        if question_lang == question_corp_deduct_exercise or question_lang == question_include_exercise:
            correct = (value_at_exercise - strike_price)*number_of_shares
            judgements = {correct:f'<p>That is correct. Because there was no readily ascertainable fair market value, income for the employee, and the associated deduction for the employer, occur when the option is exercised, under <a href="https://www.law.cornell.edu/uscode/text/26/83" target="_new" rel="noreferrer">Section 83(e)(3)</a>. The value at that time equals the difference between the total value received, that is, the value of the shares, {fm.ac(value_at_exercise)}, minus the strike price, {fm.ac(strike_price)}, times the number of shares.</p>',
                          0:'Remember: this option did not have a readily ascertainable fair market value at grant.',
                          value_at_exercise*number_of_shares:'What about the amount paid for the stock?'
                          }

        if question_lang == question_include_sale:
            correct = (value_at_sale - value_at_exercise)*number_of_shares
            judgements = {correct:'That is correct. The amount included in the sale is the gain from the sale of the stock, that is, the difference between the amount received (' + fm.ac(value_at_sale) + ') and the basis of the stock. The basis of the stock equals the amount paid for the stock (in this case, ' + fm.ac(strike_price) + ') plus the amount previously included in gross income with respect to the stock (in this case, ' + fm.ac(value_at_exercise-strike_price) + ') times the number of shares.',
                    (value_at_sale-strike_price)*number_of_shares:'This assumes the basis at sale was equal to the amount paid for the stock. What about the amount included in income when the option was exercised?',
                }
    ...
    if readily_asc_FMV == 'yes':
        ...
        if question_lang == question_corp_deduct_grant or question_lang == question_include_grant:
            ...
            judgements = {correct:'That is correct. Because the option has a readily ascertainable fair market value, there is income for the employee, and the associated deduction for the employer, when the option is granted. It is just like receiving any other property as compensation.',
        ...
        if question_lang == question_corp_deduct_exercise or question_lang == question_include_exercise:
            correct = 0
            judgements = {correct:f'<p>That is correct. Because there was a readily ascertainable fair market value, the compensation event occurred at grant, and there is no income at exercise, under under <a href="https://www.law.cornell.edu/uscode/text/26/83" target="_new" rel="noreferrer">Section 83(e)(4)</a>.</p>',
                          (value_at_exercise - strike_price)*number_of_shares:f'<p>Remember: this option had a readily ascertainable fair market value at grant. Consider <a href="https://www.law.cornell.edu/uscode/text/26/83" target="_new" rel="noreferrer">Section 83(e)(4)</a>.</p>',
                          value_at_exercise*number_of_shares:f'<p>This option had a readily ascertainable fair market value at grant. Consider <a href="https://www.law.cornell.edu/uscode/text/26/83" target="_new" rel="noreferrer">Section 83(e)(4)</a>. (Plus, what about the amount paid for the stock?)</p>'
                          }

        if question_lang == question_include_sale:
            correct = (value_at_sale - strike_price)*number_of_shares - value_of_option
            judgements = {correct:f'That is correct. The amount included in the sale is the gain from the sale of the stock, that is, the difference between the amount received ({fm.ac(value_at_sale)}) times the number of shares) and the basis of the stock. The basis of the stock equals the amount paid for the stock (in this case, {fm.ac(strike_price)} times the number of shares) plus the the amount previously included in gross income with respect to the option (in this case, {fm.ac(value_of_option)}.',
                    (value_at_sale-strike_price)*number_of_shares:'This assumes the basis at sale was equal to the amount paid for the stock. What about the amount included in income when the option was received?'
                    }
...
def unrestricted_property():
    ...
    possible_questions = [question_corp_deduct_grant,question_include_grant,question_include_sale]
    
    
    
    question_lang = random.choice(possible_questions)

    possibleanswers = [0,value_at_grant,value_at_sale,value_at_grant-amount_paid_at_grant,amount_paid_at_grant,value_at_sale-amount_paid_at_grant,value_at_sale-value_at_grant]

    if question_lang == question_corp_deduct_grant or question_lang == question_include_grant:
            correct = value_at_grant - amount_paid_at_grant
            judgements = {correct:'That is correct. Because the stock is unrestricted, the employee includes, and the employer deducts, the difference between the value at grant and the amount paid at grant.',
                          value_at_grant:'What about the amount paid for the property?',
    ...
    if question_lang == question_include_sale:
        ...
        judgements = {correct:f'<p>That is correct. The amount included due to the sale is the gain from the sale of the stock, that is, the difference between the amount received, {fm.ac(value_at_sale)}, and the basis of the stock, as required by <a href="https://www.law.cornell.edu/uscode/text/26/83" target="_new" rel="noreferrer">Section 83(a)</a>. The basis of the stock equals the amount paid for the stock, in this case, {fm.ac(amount_paid_at_grant)}, plus the amount previously included in gross income with respect to the stock, in this case, {fm.ac(value_at_grant-amount_paid_at_grant)}.</p>',
                          value_at_sale:'What about the basis of the stock?',
                          value_at_sale-amount_paid_at_grant:'What about the amount included in income when the stock was granted?'
                          }
...
def principal_res():
    ...
    class PrincResChoices:
        def __init__(self,maritalstatus,introlang,nompro,genpro,verbbuyname,verbbuypron,verbmovename,verbmovepron,threshold,verbsellname,verbsellpron,verbrent):
    ...
    type_of_problem = random.choices(['move_standard','use_own_issue','nonqualified_use'],weights = [2,2,1])[0]

    married = PrincResChoices("married",f"{person1.name} and {person2.name} are married and file jointly.",'they','their','buy','buy','move','move',fm.married.section_121_threshold,'sell','sell','rent')

    if person1.gender == 'nonbinary':    
        single = PrincResChoices("single",f"{person1.name} is single and files as a single person.",person1.nom, person1.poss, 'buys','buy','moves','move',fm.single.section_121_threshold,'sells','sell','rent')
    else: 
         single = PrincResChoices("single",f"{person1.name} is single and files as a single person.",person1.nom, person1.poss, 'buys','buys','moves','moves',fm.single.section_121_threshold,'sells','sells','rents')
    ...
    move_valid = ['to a new city because of a new job','to a new residence due to health reasons']
    ...
    if type_of_problem == 'move_standard':
        ...
        possibleanswers = [full_gain,no_basis_recovery,exclude_wrong_threshold,exclude_right_threshold,no_gain]        

        excluded_amount = right_threshold

        correct = exclude_right_threshold

        judgements = {exclude_right_threshold:f'<p>Correct. The amount of gain is {fm.ac(full_gain)}, but some of that gain is excluded from gross income by the exclusion permitted under <a href="https://www.law.cornell.edu/uscode/text/26/121" target="_new" rel="noreferrer">Section 121</a>--in this case, because the relevant marital status is {status.maritalstatus}, {fm.ac(right_threshold)}. Therefore the taxpayer includes {fm.ac(full_gain)} less {fm.ac(right_threshold)}, which equals {fm.ac(exclude_right_threshold)}.</p>',no_basis_recovery:'What about the basis? Also, can any of the gain be excluded for any reason?',full_gain:'That is the amount of gain, but can any of the gain be excluded for any reason?'}

        if exclude_right_threshold != exclude_wrong_threshold:

            judgements[exclude_wrong_threshold] ='<p>What is the correct limitation for the exclusion under <a href="https://www.law.cornell.edu/uscode/text/26/121" target="_new" rel="noreferrer">Section 121(b)</a>? Consider the taxpayer status.</p>'
    ...
    #some period of unqualified use in the previous five years - testing two-year requirement and reason for move
        ...
        proportionate_exclude_wrong_threshold = int(max(0,full_gain - wrong_excluded_amount))
        ...
        if move_reason in move_valid:
            ...
            judgements = {proportionate_exclude:f'<p>Correct. The taxpayer did not live in the residence for the full two years required by <a href="https://www.law.cornell.edu/uscode/text/26/121" target="_new" rel="noreferrer">Section 121(a)</a>, but the reason for the move is contemplated in <a href="https://www.law.cornell.edu/uscode/text/26/121" target="_new" rel="noreferrer">Section 121(c)</a>. The taxpayer lived in the property for {length_time} days, so the taxpayer can exclude a proportionate amount, {length_time}/730, of {fm.ac(right_threshold)}, as described in <a href="https://www.law.cornell.edu/cfr/text/26/1.121-3" target="_new" rel="noreferrer">Treas Reg. 1.121-3(g)</a>. Therefore the taxpayer includes {fm.ac(full_gain)} less {fm.ac(excluded_amount)}, which equals {fm.ac(proportionate_exclude)}.</p>',proportionate_exclude_wrong_threshold:'This is the right idea, given the reason for the move, but what is the correct full amount to reduce proportionately?',exclude_right_threshold:'<p>Is the full <a href="https://www.law.cornell.edu/uscode/text/26/121" target="_new" rel="noreferrer">Section 121(b)</a> amount permitted to be excluded here?</p>',no_basis_recovery:'What about the basis? Also, can any of the gain be excluded for any reason?',exclude_wrong_threshold:'<p>What is the correct limitation for the exclusion under <a href="https://www.law.cornell.edu/uscode/text/26/121" target="_new" rel="noreferrer">Section 121(b)</a>? Consider the taxpayer status. Also, is that full correct amount excluded?</p>',full_gain:'That is the amount of gain, but can any of the gain be excluded for any reason?'}

        else:

            correct = full_gain

            judgements = {proportionate_exclude:'<p>What was the cause of the move? Is it contemplated in <a href="https://www.law.cornell.edu/uscode/text/26/121" target="_new" rel="noreferrer">Section 121(c)</a>?</p>',proportionate_exclude_wrong_threshold:'<p>What was the cause of the move? Is it contemplated in <a href="https://www.law.cornell.edu/uscode/text/26/121" target="_new" rel="noreferrer">Section 121(c)</a>? Also, even if it were contemplated in Section 121(c), what would be the right amount to reduce proportionately?</p>',exclude_right_threshold:'<p>Are all the requirements of <a href="https://www.law.cornell.edu/uscode/text/26/121" target="_new" rel="noreferrer">Section 121</a> met?</p>',no_basis_recovery:'What about the basis?',exclude_wrong_threshold:'<p>Are all the requirements of <a href="https://www.law.cornell.edu/uscode/text/26/121" target="_new" rel="noreferrer">Section 121</a> met? Also, if they are, what is the correct limitation for the exclusion under Section <a href="https://www.law.cornell.edu/uscode/text/26/121" target="_new" rel="noreferrer">Section 121(b)</a>? Consider the taxpayer status.</p>',full_gain:f'<p>Correct. The taxpayer did not live in the residence for sufficient time, and the reason for the move was not a reason contemplated by <a href="https://www.law.cornell.edu/uscode/text/26/121" target="_new" rel="noreferrer">Section 121(c)</a>. Therefore the taxpayer must include the full gain of {fm.ac(full_gain)}.</p>'}
    ...
    # third type - some period of unqualified use - testing apportioning gain from sale between qualified and unqualified use
        ...
        percent_qualified = (total_years_unqualified - unqualified_years)/total_years_unqualified
        ...
        judgements = {include_total:correct_explanation,
        ...
        if include_total != no_unqualified_use_wrong_answer:
            ...
            judgements[no_unqualified_use_wrong_answer]='<p>Consider <a href="https://www.law.cornell.edu/uscode/text/26/121" target="_new" rel="noreferrer">Section 121(b)</a>. Was the property always used as a principal residence?</p>',
        
        
        if include_total != must_include_gain:
            possibleanswers.append(must_include_gain)
            judgements[must_include_gain]='It is correct that all of the gain from nonqualified use must be included. Is there any other gain that must be included? Consider the general limitation on exclusion in Section 121(b).'
...
def exclusion_COD():
    ...
    addsub = random.choices([0,1], weights = [.2,.8])
    ...
    while True:
        total_assets = total_liabilities + ((-1)**addsub[0])*fm.nearestthousand(random.random()*total_liabilities)
        if total_assets != total_liabilities and total_liabilities - total_assets != .5*loan and loan != total_liabilities - total_assets:
            break
    ...
    judgements = {end_year_excess:'<p>What is the relevant time to measure liabilities and assets to determine the amount of the <a href="https://www.law.cornell.edu/uscode/text/26/108" target="_new" rel="noreferrer">Section 108</a> insolvency exception?</p>'}
    ...
    if total_assets > total_liabilities:
        correct = loan
        judgements[correct]=f'<p>That is correct. Cancellation of indebtedness is income. <a href="https://www.law.cornell.edu/uscode/text/26/108" target="_new" rel="noreferrer">Section 108</a> provides an exception to the extent liabilities are in excess of assets, but here, assets are in excess of liabilities.</p>'
        judgements[0] ='<p>Under <a href="https://www.law.cornell.edu/uscode/text/26/61" target="_new" rel="noreferrer">Section 61(a)(11)</a>, COD is income unless an explicit statutory exclusion applies. Given that assets exceed liabilities here, does an exclusion apply?</p>'

    #insolvent, and to more than the extent of the loan
    elif total_liabilities - total_assets > loan :
        correct = 0
        judgements[correct]='<p>That is correct. Cancellation of indebtedness is generally income, but <a href="https://www.law.cornell.edu/uscode/text/26/108" target="_new" rel="noreferrer">Section 108</a> provides an exception to the extent liabilities are in excess of assets immediately prior to the loan cancellation.</p>'
        judgements[loan]='<p>Generally COD is included in gross income, but <a href="https://www.law.cornell.edu/uscode/text/26/108" target="_new" rel="noreferrer">Section 108</a> provides exceptions. What exception applies here?</p>'

    #insolvent, and less than the amount of the loan
    else:
        correct = loan - (total_liabilities - total_assets)
        judgements[correct]=f'<p>That is correct. <a href="https://www.law.cornell.edu/uscode/text/26/108" target="_new" rel="noreferrer">Section 108</a> provides an exception to the general rule of inclusion, but only to the extent that the liabilities exceed the assets immediately prior to the loan cancellation. Here, liabilities exceed assets immediately prior to the loan cancellation by {fm.ac(total_liabilities)} - {fm.ac(total_assets)} = {fm.ac(total_liabilities-total_assets)}, so that amount is excluded from gross income, and {fm.ac(correct)} is included.</p>'
        judgements[0]='<p><a href="https://www.law.cornell.edu/uscode/text/26/108" target="_new" rel="noreferrer">Section 108</a> provides an exception to the general rule of inclusion, but only to the extent that the liabilities exceed the assets.</p>'
        judgements[loan]='<p>The general rule is that COD is included in income, but consider the exceptions in <a href="https://www.law.cornell.edu/uscode/text/26/108" target="_new" rel="noreferrer">Section 108</a>.</p>'
        judgements[total_liabilities-total_assets]='This is the amount that is *excluded*, but what amount is included?'
...
def qual_empl_disc():
    ...
    benefit_dict = {'property':['books','clothing','hardware'],'services':['house cleaning','website design','haircuts']}
    
    benefit = random.choice(['property','services'])
    
    type_store = random.choice(benefit_dict[benefit])
    
    type_job = random.choice(['provide marketing and publicity with respect to','sell'])
    
    purchase_fmv = random.randint(5,30)*50
    while True:
        purchase_cost = fm.generate_random_item_hund(purchase_fmv,40,90)
        if purchase_cost != .5*purchase_fmv and purchase_cost < purchase_fmv:
            break
    ...
    while True:
        ...
        if benefit == 'services':
            ...
            relevant_percent_lang = f"the gross profit percentage, which equals {relevant_percent} percent"
    ...
    possibleanswers = [total_include,gross_income_no_exclude,purchase_fmv,exclude_amount,0]
    ...
    judgements={correct:correct_explain,
...
def gift():
    ...
    property_list = ['car','luxury watch','rare book','rare coin','fancy item of clothing']
    ...
    income_dict = {correct:f'Correct. {person2.name} includes nothing in gross income from this transaction. {person1.name} gave the {property_1} out of detached and disinterested generosity. It was therefore a gift, and gifts are excluded from gross income under Section 102.'}
    
    deduction_dict = {correct:f'Correct. {person1.name} is not allowed a deduction for this transfer. {person1.name} gave the {property_1} out of detached and disinterested generosity, and it was therefore a gift, but no deduction is allowed for gifts to individuals.'}
...
def charitable_donation(type_problem='random'):
    ...
    type_gain = random.choice(['long-term capital gain','short-term capital gain','ordinary income'])
    ...
    if type_donation == 'services':
        ...
        correct_explanation = f'<p>Correct. {income_sentence_true} {services_sentence}</p>'
        ...
        if not itemizes:
            ...
            judgements={'No gain, no deduction':correct_explanation,
            ...
            correct_explanation = f'<p>Correct. {gain_sentence_true} {correct_amount_sentence}</p>'
...
def qri():
    ...
    class QRIChoices:
        def __init__(self,introlang,nompro,genpro,buyproper,buypronoun,financeproper,financepronoun,itemizeproper,itemizepronoun):
            self.introlang = introlang
            self.nompro = nompro
            self.genpro = genpro
    ...
    married = QRIChoices(f'{person1.name} and {person2.name} are married and file jointly.','they','their','buy','buy','finance','finance','itemize','itemize')

    if person1.gender == 'nonbinary':    
        single = QRIChoices(f'{person1.name} is single and files as a single person.', person1.nom, person1.poss, 'buys','buy','finances','finance','itemizes','itemize')
    else:
        single = QRIChoices(f'{person1.name} is single and files as a single person.', person1.nom, person1.poss, 'buys','buy','finances','finances','itemizes','itemizes')
    ...
    while True:
        mortgage_amount = int(fm.nearestthousand((random.randint(50,90)/100)*purchase_price))
    ...
    judgements = {mortgage_amount:'The interest is what is deductible, not the principal amount.',fm.qri_limit:'The interest is what is deductible, not the amount of acquisition indebtedness.',annual_interest_old_limit:'What is the current limitation for the amount that can be considered acquisition indebtedness?'}

    if mortgage_amount > fm.qri_limit:
        judgements[annual_interest]='That is the annual interest, but what about the limitation on acquisition indebtedness?'
        judgements[annual_interest_limit]=f'Correct! Only the interest with respect to {fm.ac(fm.qri_limit)} is permitted to be deducted.' 

        correct = annual_interest_limit

    else:
        judgements[annual_interest]=f'Correct! The interest with respect to up to {fm.ac(fm.qri_limit)} of acquisition indebtedness is permitted to be deducted, and the amount of the mortgage does not exceed {fm.ac(fm.qri_limit)}.'
        judgements[annual_interest_limit]='That is the interest amount times the annual limit for acquisition indebtedness, but what is the actual principal of the loan?'

        correct = annual_interest

    possibleanswers = [fm.qri_limit,purchase_price,mortgage_amount,down_payment,annual_interest,annual_interest_limit,annual_interest_old_limit,annual_interest_full_amount]
...
def property_exchange():
    ...
    class ExchangePerson:
      def __init__(self, name, property_exchanged,property_received,basis_exchanged_property,gain_loss):
        self.name = name
    ...
    first_person = ExchangePerson(person1.name,property_1,property_2,property_1_purchase_price,person_1_gain)
    second_person = ExchangePerson(person2.name,property_2,property_1,property_2_purchase_price,person_2_gain)
...
def basis_problems(type_problem='random'):
    ...
    if type_problem == 'gift':
        ...
        if question_lang == transferor_gain_q:
            ...
            if transferor_gain == 0:
                ...
                judgements[transferor_gain]='<p>Correct! The donor recognizes gain only to the extent the value of the sold portion exceeds the basis of the property under <a href="https://www.law.cornell.edu/cfr/text/26/1.1001-1" target="_new" rel="noreferrer">Section 1.1001-1(e)</a> and does not recognize loss. Essentially, the donor recovers basis first. Here, the basis exceeds the sale price, so the transferor recognizes no gain.</p>'
                ...
                judgements[transferor_gain]='<p>Correct! The donor recognizes gain only to the extent the value of the sold portion exceeds the basis of the property under <a href="https://www.law.cornell.edu/cfr/text/26/1.1001-1" target="_new" rel="noreferrer">Section 1.1001-1(e)</a>. Essentially, the donor recovers basis first.</p>'
        ...
        judgements[transferor_gain]='<p>Correct! Under <a href="https://www.law.cornell.edu/uscode/text/26/1011" target="_new" rel="noreferrer">Section 1011(b)</a> the donor recovers basis proportionately.</p>'
        judgements[gain_to_transferor_basis_first]='<p>How is basis recovered on a part donation / part sale? Consider <a href="https://www.law.cornell.edu/uscode/text/26/1011" target="_new" rel="noreferrer">Section 1011(b)</a>. Compare the basis recovery for property that is transferred through part gift / part sale.</p>'
        ...
        if question_lang == recipient_gain_q:
            ...
            judgements[later_sale_price - recipient_basis_transferred]='<p>What happens to the basis of an asset that is transferred due to death? Consider <a href="https://www.law.cornell.edu/uscode/text/26/1014" target="_new" rel="noreferrer">Section 1014(a)</a>.</p>'
        ...
        if question_lang == recipient_gain_q:
            if asset_value < transferor_basis and later_sale_price < transferor_basis:
                ...
                judgements[recipient_gain]=f'Correct! The basis of the property is always transferred between spouses. {person2.name} therefore recognizes {fm.ac(later_sale_price)} - {fm.ac(recipient_basis)} = {fm.ac(recipient_gain)}.'
    ...
    while len(possibleanswers) < 5:    
        (possibleanswers, judgements) = fm.random_answer_pot(possibleanswers,judgements,3,40,140)
...
def liabilities():
    ...
    debt_type = random.choice([['nonrecourse','takes the building subject to the debt'],['recourse','assumes the debt']])
    ...
    if cash_paid == 0:
        ...
        cash_paid_lang = f' {person2.name} also pays {person1.name} {fm.ac(cash_paid)} in cash.'
    ...
    judgements = {gain_realized_cash_only:'What is included in amount realized?',all_cod:'<p>Consider <a href="https://www.law.cornell.edu/cfr/text/26/1.1001-2" target="_new" rel="noreferrer">Treas. Reg. 1.1001-2(a)(1)</a>.</p>'}
    ...
    if debt > building_fmv: #debt is in excess of fair market value
        ...
        if debt_type[0] == 'recourse':
            ...
            judgements[gain_realized_nonrecourse] = 'This would be correct for nonrecourse debt. But debt relief in excess of the fair market value of the property has a different treatment for recourse debt.'
            judgements[gain_realized_recourse] = f'Correct. Because this is recourse debt, debt relief up to fair market value is included in amount realized; debt relief in excess of fair market value is treated as cancellation of indebtedness income. In this problem, {fm.ac(building_fmv)} is included in amount realized, for a total of {fm.ac(abs(gain_realized_recourse_numb))} of {fm.gainword(gain_realized_recourse_numb)}. The excess of the debt over the fair market value, that is, {fm.ac(debt)} minus {fm.ac(building_fmv)}, is cancellation of indebtedness income.'
    
        if debt_type[0] == 'nonrecourse':
            
            correct = gain_realized_nonrecourse
                
            judgements[gain_realized_nonrecourse]='Correct. Because this is nonrecourse debt, all of the debt relief is included in amount realized.'
            judgements[gain_realized_recourse]='This would be correct for recourse debt. But this is nonrecourse debt.'
        ...
        if debt_type[0] == 'recourse':
            ...
            judgements[gain_realized_nonrecourse]='Correct. Because the amount of debt relief is less than the fair market value of the building, all of the debt relief is included in amount realized.'
...
def like_kind():
    ...
    use_lang_choice = random.choices([personal_use_lang,business_use_lang],weights=(1,4))[0]
    ...
    land_boot_debt_swap = max(0,land_debt-building_debt) #this is just the debt netted, because the person who gives up land doesn't give up any other boot
    ...
    basis_total_building_recip_like_kind = land_basis + building_debt - land_debt + land_gain_recognized_like_kind     #in a like-kind exchange, total basis equals the land basis plus the net debt relief plus the gain recognized
    
    basis_machine_recip = machine_fmv #allocate basis first to the boot
    basis_building_recip_like_kind = basis_total_building_recip_like_kind - basis_machine_recip
    basis_building_recip_personal_use = building_fmv
    basis_building_recip_like_kind_no_debt = basis_building_recip_like_kind + land_debt - building_debt
    ...
    building_gain_recognized_like_kind = min(building_gain_realized,building_boot_debt_swap)
    total_building_gain_recognized = machine_gain_realized + building_gain_recognized_like_kind
    
    #here are some wrong answers:
    building_gain_recognized_like_kind_no_netting = min(building_gain_realized,building_debt)
    
    building_boot_debt_swap_netting_no_mach = max(0,building_debt-land_debt)
    building_gain_recognized_like_kind_netting_no_mach = min(building_gain_realized,building_boot_debt_swap_netting_no_mach)
    
    #now, basis
    
    basis_land_recip_in_land = building_basis + machine_basis + total_building_gain_recognized + land_debt - building_debt
    ...
    disregard_debt_basis_building_person = building_basis + machine_basis + total_building_gain_recognized
    ...
    possible_questions = [building_person_gain_q,land_person_gain_q,building_person_basis_q,land_person_basis_q]
    ...
    if question_lang == land_person_gain_q:
        ...
        if use_lang_choice == 'personal use': #personal use--does not matter what the options are

            possibleanswers.append(land_gain_realized)
            land_person_gain = land_gain_realized        
            correct=land_person_gain
            
            judgements[land_person_gain]=f'Correct. This is not a like-kind exchange, because {person2.name} did not use the land in a trade or business.'
            if land_gain_recognized_like_kind != land_person_gain:
                judgements[land_gain_recognized_like_kind]=f'What are the requirements for a like-kind exchange? Are those requirements met here?',
            judgements[0]=f'Is this a like-kind exchange? Also, even if it were a like-kind exchange, is any boot received?'
            ...
            if land_person_gain != land_gain_realized:
                possibleanswers.append(land_gain_realized)
                judgements[land_gain_realized]='Is all the gain recognized on this transaction? Why not?'
                judgements[land_person_gain]=f'Correct! This is a like-kind exchange. With respect to the land, {person2.name} realizes {fm.ac(land_gain_realized)}, which will be recognized to the extent of boot received--here, the fair market value of the machine, {fm.ac(machine_fmv)}.'
                
            elif land_person_gain == land_gain_realized:
                judgements[land_person_gain]=f'Correct! This is a like-kind exchange. With respect to the land, {person2.name} realizes {fm.ac(land_gain_realized)}, which will be recognized to the extent of boot received. Here, the fair market value of the machine, {fm.ac(machine_fmv)}, exceeds the gain realized, so the full amount of gain is recognized, notwithstanding that this is a like-kind exchange.'
            
        elif land_boot_debt_swap == 0: # like kind exchange with debt but no net debt relief
        
            land_person_gain = land_gain_recognized_like_kind
            correct=land_person_gain
            
            possibleanswers = possibleanswers + [land_gain_recognized_like_kind_no_netting]     

            
            if land_person_gain != land_gain_realized:
                possibleanswers.append(land_gain_realized)
                judgements[land_gain_realized]='Is all the gain recognized on this transaction? Why not?'
                judgements[land_person_gain]=f'Correct! This is a like-kind exchange, so gain realized, {fm.ac(land_gain_realized)}, is recognized to the extent of boot received. Here, the only boot received is the machine, because only *net* debt relief is boot.'
                
            elif land_person_gain == land_gain_realized:
                judgements[land_person_gain]=f'Correct! This is a like-kind exchange. With respect to the land, {person2.name} realizes {fm.ac(land_gain_realized)}, which will be recognized to the extent of boot received. Here, the only boot received is the machine, because only *net* debt relief is boot. The fair market value of the machine, {fm.ac(machine_fmv)}, exceeds the gain realized, so the full amount of gain is recognized, notwithstanding that this is a like-kind exchange.'                

            judgements[land_gain_recognized_like_kind_no_netting]='Remember: only *net* debt relief is boot.'
            ...
            if land_person_gain != land_gain_realized:
                possibleanswers.append(land_gain_realized)
                judgements[land_gain_realized]='Is all the gain recognized on this transaction? Why not?'
                judgements[land_person_gain]=f'Correct! This is a like-kind exchange, so gain realized, {fm.ac(land_gain_realized)}, is recognized to the extent of boot received. Here, {person2.name} receives boot equal to the fair market value of the machine, {fm.ac(machine_fmv)}, plus the net debt relief, that is, the excess of {fm.ac(land_debt)} over {fm.ac(building_debt)}.'
                
            elif land_person_gain == land_gain_realized:
                judgements[land_person_gain]=f'Correct! This is a like-kind exchange. With respect to the land, {person2.name} realizes {fm.ac(land_gain_realized)}, which will be recognized to the extent of boot received. Here, {person2.name} receives boot equal to the fair market value of the machine, {fm.ac(machine_fmv)}, plus the net debt relief, that is, the excess of {fm.ac(land_debt)} over {fm.ac(building_debt)}. The boot received thus exceeds the gain realized, so the full amount of gain is recognized, notwithstanding that this is a like-kind exchange.'
    ...
    if question_lang == building_person_basis_q:
        ...
        if choose_debt == 0:
            ...
            judgements[building_person_basis]=f"Correct. The basis in the land equals {person1.name}'s basis in the building, {fm.ac(building_basis)}, plus {person1.poss} basis in the machine, {fm.ac(machine_basis)}, plus total gain or loss recognized, {fm.ac(total_building_gain_recognized)}."
    ...
    if question_lang == land_person_basis_q:
        ...
        possibleanswers = [basis_total_building_recip_like_kind,basis_building_recip_like_kind,basis_building_recip_personal_use]
        
        if use_lang_choice == 'personal use': #personal use--does not matter what the options are
        
            correct=basis_building_recip_personal_use
            
            judgements[basis_building_recip_personal_use]=f'Correct. This is not a like-kind exchange, so {person2.name} recognizes all realized gain and has a cost basis in the building.'
            ...
            judgements[basis_building_recip_personal_use]=f'Does {person2.name} recognize all realized gain?'
    ...
    problem = problem_part_1 + debt_lang_building + problem_part_2 + debt_lang_land + question_lang
...
def installment_sale_plain():
    ...
    second_payment_date = fm.month_day(problem_date_number) + ', ' + str(second_payment_year)
    ...
    gain_first_payment_basis_last = initial_payment - max(0,property_basis-second_payment)
    ...
    gain_second_payment_full_note_as_payment = realized_gain - gain_first_payment_full_note_as_payment
    ...
    if question_lang == first_payment_gain_q:
        ...
        possibleanswers = [gain_first_payment_basis_first,gain_first_payment_basis_last,gain_first_payment_full_note_as_payment,gain_first_payment_prorata]

        judgements = {correct:f'Correct! The note itself is not considered a payment, so the only payment in the year of the sale is {fm.ac(initial_payment)}. A proportionate amount of each payment is treated as amount recognized, specifically the fraction equal to the total gain divided by the contract price--that is, {fm.ac(realized_gain)} divided by {fm.ac(property_fmv)}, or {fm.as_percent(realized_gain/property_fmv)}.<br><br>Equivalently, you can think of this as basis being recovered proportionate to the payment. That is, because {fm.ac(initial_payment)} divided by {fm.ac(property_fmv)}, or {fm.as_percent(initial_payment/property_fmv)}, of the payments are received, the same percentage of gain realized is recognized.',gain_first_payment_basis_first:'This would be correct if all of the basis could be put toward the initial payment--that is, if basis were recovered first. But that is not the approach in installment sales.',gain_first_payment_basis_last:'This would be correct if basis were allocated first to the payments on the note--that is, if basis were recovered last. But that is not the approach in installment sales.',gain_first_payment_full_note_as_payment:'This would be correct if the note were treated as payment in the year it is received. But that is not the approach in installment sales.'}

    if question_lang == second_payment_gain_q:
        correct = gain_second_payment_prorata

        possibleanswers = [gain_second_payment_basis_first,gain_second_payment_basis_last,gain_second_payment_full_note_as_payment,gain_second_payment_prorata]

        judgements = {correct:f'Correct! The note itself is not considered a payment; gain on the amount of the note is not recognized until the note is paid. Thus the payment in {str(second_payment_year)} is {fm.ac(second_payment)}. A proportionate amount of each payment is treated as amount recognized, specifically the fraction equal to the total gain divided by the contract price--that is, {fm.ac(realized_gain)} divided by {fm.ac(property_fmv)}, or {fm.as_percent(realized_gain/property_fmv)}.<br><br>Equivalently, you can think of this as basis being recovered proportionate to the payment. That is, because {fm.ac(second_payment)} divided by {fm.ac(property_fmv)}, or {fm.as_percent(second_payment/property_fmv)}, of the payments are received, the same percentage of gain realized is recognized.',gain_second_payment_basis_first:'This would be correct if all of the basis had been put toward the initial payment--that is, if basis were recovered first. But that is not the approach in installment sales.',gain_second_payment_basis_last:'This would be correct if basis were allocated first to the payments on the note--that is, if basis were recovered last. But that is not the approach in installment sales.',gain_second_payment_full_note_as_payment:'This would be correct if the note were treated as payment in the year it is received. But that is not the approach in installment sales.'}

    problem = problem_lang_intro + problem_lang_other_comp + problem_lang_provides_note + question_lang
...
def installment_sale_debt():
    ...
    gain_first_payment_qual_debt = int(initial_payment_qual_debt * (realized_gain/contract_price_excluded_debt))
    ...
    gain_first_payment_exclude_all_debt = int(initial_payment_exclude_all_debt * (realized_gain/contract_price_exclude_all_debt))
    gain_second_payment_exclude_all_debt = realized_gain - gain_first_payment_exclude_all_debt
    ...
    gain_second_payment_basis_first = int(realized_gain - gain_first_payment_basis_first)
    gain_second_payment_basis_last = realized_gain - gain_first_payment_basis_last
    gain_second_payment_full_note_as_payment = realized_gain - gain_first_payment_full_note_as_payment
    ...
    if choose_qualifying == 'qualified debt':
        ...
        if question_lang == first_payment_gain_q:
            ...
            possibleanswers = [gain_first_payment_qual_debt,gain_first_payment_basis_last,gain_first_payment_full_note_as_payment,gain_first_payment_prorata,random_payment]
    
            judgements = {correct:f'Correct! Because the debt was not acquired in contemplation of the disposition of the property, this is qualifying indebtedness. Therefore the amount of the debt relief is excluded from payment received and from contract price to the extent of basis.',gain_first_payment_prorata:'This would be correct if this were not qualifying indebtedness.',gain_first_payment_basis_last:'This would be correct if this were not qualifying indebtedness and if basis were allocated first to the payments on the note--that is, if basis were recovered last. But that is not the approach in installment sales.',gain_first_payment_full_note_as_payment:'This would be correct if this were not qualifying indebtedness and the note were treated as payment in the year it is received. But that is not the approach in installment sales.',random_payment:'This number was randomly generated.'}
        
            if initial_payment > property_basis:
                possibleanswers.append(gain_first_payment_exclude_all_debt)
    
                judgements[gain_first_payment_exclude_all_debt] = 'It is true this is qualifying indebtedness, but debt can be excluded only up to basis.'
            
        if question_lang == second_payment_gain_q:
            
            correct = gain_second_payment_qual_debt
            
            possibleanswers = [gain_second_payment_qual_debt,gain_second_payment_basis_last,gain_second_payment_full_note_as_payment,gain_second_payment_prorata,random_payment]
    
            judgements = {correct:f'Correct! Because the debt was not acquired in contemplation of the disposition of the property, this is qualifying indebtedness. Therefore the amount of the debt relief is excluded from payment received and from contract price to the extent of basis.',gain_second_payment_prorata:'This would be correct if this were not qualifying indebtedness.',gain_second_payment_basis_last:'This would be correct if this were not qualifying indebtedness and if basis were allocated first to the payments on the note--that is, if basis were recovered last. But that is not the approach in installment sales.',gain_second_payment_full_note_as_payment:'This would be correct if this were not qualifying indebtedness and the note were treated as payment in the year it is received. But that is not the approach in installment sales.',random_payment:'This number was randomly generated.'}
        
            if initial_payment > property_basis:
                
                possibleanswers.append(gain_second_payment_exclude_all_debt)
    
                judgements[gain_second_payment_exclude_all_debt] = 'It is true this is qualifying indebtedness, but debt can be excluded only up to basis.'
        ...
        if question_lang == first_payment_gain_q:
            ...
            judgements = {correct:f'Correct! This is not qualifying indebtedness, and so debt relief is not excluded from payment received or from contract price. The note itself is not considered a payment, so the only payment in the year of the sale is {fm.ac(initial_payment)}. Basis is recovered proportionate to the payment. That is, because {fm.ac(initial_payment)} divided by {fm.ac(property_fmv)}, or {fm.as_percent(initial_payment/property_fmv)}, of the payments are received, the same percentage of gain realized is recognized.',gain_first_payment_qual_debt:'This would be correct if this were qualifying indebtedness.',gain_first_payment_basis_last:'It is true this is not qualifying indebtedness, and this would be correct if basis were allocated first to the payments on the note--that is, if basis were recovered last. But that is not the approach in installment sales.',gain_first_payment_full_note_as_payment:'It is true this is not qualifying indebtedness, and this would be correct if the note were treated as payment in the year it is received. But that is not the approach in installment sales.',random_payment:'This number was randomly generated.'}
        ...
        if question_lang == second_payment_gain_q:
            ...
            judgements = {correct:f'Correct! This is not qualifying indebtedness, and so debt relief is not excluded from payment received or from contract price. The note itself is not considered a payment; gain on the amount of the note is not recognized until the note is paid. Thus the payment in {str(second_payment_year)} is {fm.ac(second_payment)}. Basis is recovered proportionate to the payment. That is, because {fm.ac(second_payment)} divided by {fm.ac(property_fmv)}, or {fm.as_percent(second_payment/property_fmv)}, of the payments are received, the same percentage of gain realized is recognized.',gain_second_payment_qual_debt:'This would be correct if this were qualifying indebtedness.',gain_second_payment_basis_last:'This would be correct if basis were allocated first to the payments on the note--that is, if basis were recovered last. But that is not the approach in installment sales.',gain_second_payment_full_note_as_payment:'This would be correct if the note were treated as payment in the year it is received. But that is not the approach in installment sales.',random_payment:'This number was randomly generated.'}

    problem = problem_lang_intro + problem_lang_other_comp + problem_lang_addl + problem_lang_provides_note + question_lang
...
def installment_sale_related_party():
    ...
    if person2.name in names.malenames:
        relatives_267 = ['brother','husband','father','grandfather','great-grandfather','son','grandson']
        relatives_other = ['cousin','uncle','nephew']
        
    else:    
        relatives_267 = ['sister','wife','mother','grandmother','great-grandmother','daughter','granddaughter']
    ...
    third_party_sale_date = fm.full_date(fm.date_after(problem_date_number,soonest=how_far_after,latest=how_far_after))
    ...
    resale_price_as_payment = max(0,(min(property_fmv,resale_price) - initial_payment))
    ...
    amt_included_second_payment_with_453 = max(0,second_payment - resale_price_as_payment)    
    gain_second_payment_pro_rata_with_453 = int(realized_gain*amt_included_second_payment_with_453/property_fmv)
    ...
    question_lang = random.choice([first_payment_gain_q,second_payment_gain_q,resale_gain_q]) 
    
    if question_lang == first_payment_gain_q:
        correct = gain_first_payment_prorata

        possibleanswers = [gain_first_payment_basis_first,gain_first_payment_basis_last,gain_first_payment_full_note_as_payment,gain_first_payment_prorata,random_payment]

        judgements= {correct:f'Correct! The note itself is not considered a payment, so the only payment in the year of the sale is {fm.ac(initial_payment)}. A proportionate amount of each payment is treated as amount recognized, specifically the fraction equal to the total gain divided by the contract price--that is, {fm.ac(realized_gain)} divided by {fm.ac(property_fmv)}, or {fm.as_percent(realized_gain/property_fmv)}.<br><br>Equivalently, you can think of this as basis being recovered proportionate to the payment. That is, because {fm.ac(initial_payment)} divided by {fm.ac(property_fmv)}, or {fm.as_percent(initial_payment/property_fmv)}, of the payments are received, the same percentage of gain realized is recognized.',gain_first_payment_basis_first:'This would be correct if all of the basis could be put toward the initial payment--that is, if basis were recovered first. But that is not the approach in installment sales.',gain_first_payment_basis_last:'This would be correct if basis were allocated first to the payments on the note--that is, if basis were recovered last. But that is not the approach in installment sales.',gain_first_payment_full_note_as_payment:'This would be correct if the note were treated as payment in the year it is received. But that is not the approach in installment sales.',random_payment:'This number was randomly generated.'}
        
    if question_lang == resale_gain_q:
        
        if relationship in relatives_267:
        
            possibleanswers = [gain_resale_prorata,0,random_payment,gain_first_payment_basis_first,gain_first_payment_basis_last,gain_first_payment_full_note_as_payment]
            
            if how_far_after < 730:
            
                correct = gain_resale_prorata
            
                judgements = {correct:f'<p>Correct! This is a relationship described in <a href="https://www.law.cornell.edu/uscode/text/26/267" target="_new" rel="noreferrer">Section 267</a>, and the sale to {person3.name} happened not more than two years after the initial sale from {person1.name} to {person2.name}, so <a href="https://www.law.cornell.edu/uscode/text/26/453" target="_new" rel="noreferrer">Section 453(e)</a> applies. The payment included is {fm.ac(resale_price_as_payment)}, and a proportionate amount of that payment, {fm.ac(gain_resale_prorata)}, is income.</p>',0:f'<p>This is a relationship described in <a href="https://www.law.cornell.edu/uscode/text/26/267" target="_new" rel="noreferrer">Section 267</a>, and the the sale to {person3.name} happened less not more than two years after the initial sale from {person1.name} to {person2.name}. Consider <a href="https://www.law.cornell.edu/uscode/text/26/453" target="_new" rel="noreferrer">Section 453(e)</a>.</p>',random_payment:'This number was randomly generated.'}
                
                if resale_price > second_payment:
                    
                    possibleanswers.append(full_resale_as_payment)
                    
                    judgements[full_resale_as_payment]=f'How much of the sale price to {person3.name} is considered a payment on the note? Consider 453(e)(3).'

            if how_far_after > 730:
                
                correct = 0
                
                judgements = {correct:f'<p>Correct! This is a relationship described in <a href="https://www.law.cornell.edu/uscode/text/26/267" target="_new" rel="noreferrer">Section 267</a>, so <a href="https://www.law.cornell.edu/uscode/text/26/453" target="_new" rel="noreferrer">Section 453(e)</a> could apply. But because the sale to {person3.name} happened more than two years after the initial sale from {person1.name} to {person2.name}, it exceeds the two-year cutoff described in 453(e)(2), so there is no deemed payment.</p>', gain_resale_prorata:f'<p>This is a relationship described in <a href="https://www.law.cornell.edu/uscode/text/26/267" target="_new" rel="noreferrer">Section 267</a>, so <a href="https://www.law.cornell.edu/uscode/text/26/453" target="_new" rel="noreferrer">Section 453(e)</a> could apply. But how long did after the sale from {person1.name} to {person2.name} did the sale to {person3.name} occur? Consider 453(e)(2).</p>',random_payment:'This number was randomly generated.'}
        
                if resale_price > second_payment:
                    
                    possibleanswers.append(full_resale_as_payment)
                    
                    judgements[full_resale_as_payment]=f'<p>Does <a href="https://www.law.cornell.edu/uscode/text/26/453" target="_new" rel="noreferrer">Section 453(e)</a> apply? How long after the initial sale did the sale to {person3.name} take place? Moreover, even if it did apply, how much of the sale price to {person3.name} would be considered a payment on the note? Consider 453(e)(3).</p>'

        else:
            
            correct = 0
            
            possibleanswers = [gain_resale_prorata,0,random_payment,full_resale_as_payment]
            
            judgements = {correct:'<p>Correct! This is not a relationship described in <a href="https://www.law.cornell.edu/uscode/text/26/267" target="_new" rel="noreferrer">Section 267</a>.</p>',gain_resale_prorata:'<p>Is this a relationship described in <a href="https://www.law.cornell.edu/uscode/text/26/267" target="_new" rel="noreferrer">Section 267</a>?</p>',full_resale_as_payment:'<p>Is this a relationship described in <a href="https://www.law.cornell.edu/uscode/text/26/267" target="_new" rel="noreferrer">Section 267</a>?</p>',random_payment:'This number was randomly generated.'}
...
def installment_sale_like_kind():
    ...
    gain_first_payment_prorata_no_like_kind = int(building_gain_realized * (initial_payment_no_like_kind/building_fmv))
    ...
    gain_first_year_prorata_like_kind = int(cash_amt * total_gain_recognized_like_kind / total_boot)
    ...
    use_lang_choice_person1 = random.choices([personal_use_lang_1,business_use_lang_1],weights=(1,4),k=1)[0]
    ...
    problem = problem_lang_intro + problem_lang_person1_use + problem_lang_person2_use + question_lang

    possibleanswers = [gain_first_year_prorata_like_kind,building_gain_realized,random_payment,gain_first_payment_prorata_no_like_kind]

    judgements = {random_payment:'This number was randomly generated.',building_gain_realized:'What is the signficance of the note? Consider <a href="https://www.law.cornell.edu/uscode/text/26/453" target="_new" rel="noreferrer">Section 453</a>.'}
    ...
    if use_lang_choice_person1 == business_use_lang_1:
        ...
        judgements[gain_first_year_prorata_like_kind]=f'<p>Correct! This is a like-kind exchange. {person1.name} realizes {fm.ac(building_gain_realized)} on the transaction. Because this is a like-kind exchange, {person1.name} recognizes that gain to the extent of boot received. {boot_recognized_lang} The installment method controls when that amount will be recognized, as described in <a href="https://www.law.cornell.edu/uscode/text/26/453" target="_new" rel="noreferrer">Section 453(f)(6)</a>. Conceptually, the basis in the building of {fm.ac(building_basis)} goes first to the like-kind portion. {amt_lang}</p>'
            
        if use_lang_choice_person2 == personal_use_lang_2:
        
            judgements[gain_first_payment_prorata_no_like_kind]=f'<p>How do the like-kind rules of <a href="https://www.law.cornell.edu/uscode/text/26/1031" target="_new" rel="noreferrer">Section 1031</a> interact with the installment sale rules? For example, is {person2.name}\'s planned use of the property relevant to your analysis?</p>'
           
            
        else:
            
            judgements[gain_first_payment_prorata_no_like_kind]=f'<p>How do the like-kind rules of <a href="https://www.law.cornell.edu/uscode/text/26/1031" target="_new" rel="noreferrer">Section 1031</a> interact with the installment sale rules?</p>'
            
        
    else:
        correct = gain_first_payment_prorata_no_like_kind
        
        judgements[gain_first_payment_prorata_no_like_kind]=f'Correct! Because {person1.name} will use the land for personal use, this is not a like-kind exchange. Therefore the regular installment sale rules apply, and the basis is recovered pro rata.'
        judgements[gain_first_year_prorata_like_kind]='Is this a like-kind exchange? How will the received property be used?'
...
def depreciation_question(type_problem='random'):
    ...
    if asset.listed == False:
        period_lang = f" The class life of the {asset.name} is {asset.class_life} years."
    ...
    year_depreciation = int(round(fm.depreciate_asset(asset,years_passed,initial_basis,sold)[1]))
    ...
    possibleanswers_real = [year_depreciation,no_disposal,straight_line_no_disposal,class_life_straight_line]
    random_answer = random.randint(min(possibleanswers_real),max(possibleanswers_real))
    
    possibleanswers = [year_depreciation,straight_line_no_disposal,class_life_straight_line,random_answer]
    
    judgements = {straight_line_no_disposal:'<p>What is the method of depreciation that applies to this asset? Consider <a href="https://www.law.cornell.edu/uscode/text/26/168" target="_new" rel="noreferrer">Section 168(b)</a>.</p>',random_answer:"This number was randomly generated.",class_life_straight_line:'<p>Is the class life the same as the recovery period? Also, what is the method of depreciation that applies to this asset? Consider <a href="https://www.law.cornell.edu/uscode/text/26/168" target="_new" rel="noreferrer">Section 168(b)</a>.</p>'}
    ...
    if sold == True:
        ...
        judgements[year_depreciation]=f'<p>Correct! To obtain this answer, apply the correct schedule from <a href="/assets/RevProc87-57.pdf" target="_new">Rev. Proc. 87-57</a> to the initial basis. Specifically, this property has a recovery period of {asset.recovery_period} years, starting and ending with a half year of depreciation, and this is year {years_passed+1} of that time. Moreover, the {asset.name} was sold, and there is deemed to be half a year of depreciation available in the year sold.</p>'
        judgements[no_disposal]=f'<p>This would be correct if the {asset.name} had not been sold. But what about the convention, as described in <a href="https://www.law.cornell.edu/uscode/text/26/168" target="_new" rel="noreferrer">Section 168(d)</a>?</p>'
        judgements[straight_line_no_disposal]='<p>What is the method of depreciation that applies to this asset? Consider <a href="https://www.law.cornell.edu/uscode/text/26/168" target="_new" rel="noreferrer">Section 168(b)</a>. Also consider the relevant convention, as in Section 168(d).</p>'
        ...
        judgements[straight_line_no_disposal]='<p>What is the method of depreciation that applies to this asset? Consider <a href="https://www.law.cornell.edu/uscode/text/26/168" target="_new" rel="noreferrer">Section 168(b)</a>.</p>'
...
def cap_gain_netting():
        ...
        def create_stock_facts(loss_gain,long_or_short,name_list=[]):
            ...
            stock_dict = {'corp_name':corp_name,
        ...
        for stock_dict in stock_dict_list:
            ...
            asset_purchase_lang = asset_purchase_lang + f"<br>{stock_dict['problem_facts']}<br>"
            ...
            if identifier_pair == ['gain','long']: 
                    ...
                    print(f'The new long term loss is {stock_dict["gain_loss_amount"]} and long term capital loss is now {LTCL}')
                    ...
                    print(f'The new loss is {stock_dict["gain_loss_amount"]} and short term capital loss is now {STCL}')
        ...
        if type_question == 'capital_gains':
            ...
            while True:    
                ...
                if net_CG != 0:   
                    ...
                    random_answer = fm.generate_random_item(net_all_gains_losses,70,120)+500
                else:
            ...
            judgements = {correct:f"That is correct! The amount taxed at the preferential rate is the net capital gain. The net capital gain is the excess of net long term capital gain, which is {fm.ac(net_LTCG)}, over net short term capital loss, which is {fm.ac(net_STCL)}. More specifically: {answer_lang}",random_answer:'This answer was randomly generated.'}
                
            if net_CG != net_all_gains:
                
                    judgements = fm.merge(judgements,{net_all_gains:'<p>You netted all the capital gains. How do the capital losses figure in? Consider <a href="https://www.law.cornell.edu/uscode/text/26/1222" target="_new" rel="noreferrer">Section 1222(11)(b)</a>, and work back through the definitions of the various terms.</p>'})
                    
            if net_all_gains_losses not in [net_CG,net_all_gains]:
                    
                    judgements = fm.merge(judgements,{net_all_gains_losses:'<p>You netted all the gains and losses. But netting all the capital gains and losses does not result in net capital gain, which is a defined term. In particular, consider <a href="https://www.law.cornell.edu/uscode/text/26/1222" target="_new" rel="noreferrer">Section 1222(11)(b)</a>, and work back through the definitions of the various terms.</p>'})
                ...
                number_list = [loss_carryforward,loss_carryforward_soak_up_all_OI,loss_carryforward_no_3000,usable_loss,0]
                while True:
                
                    if loss_carryforward != 0:
                        random_answer = fm.generate_random_item(loss_carryforward,80,120)+500

                    else: random_answer = 500*random.randrange(11,61,2)
                    
                    if random_answer not in number_list:
                        break
            
                possibleanswers = [loss_carryforward,loss_carryforward_soak_up_all_OI,loss_carryforward_no_3000,usable_loss,random_answer,0]

                correct = loss_carryforward
                
                if correct == 0: 
                    
                    judgements = {correct:f'<p>That is correct! {person1.name} is an individual, so the losses that may be used in {fm.current_year} equal the total amount of capital gains plus up to $3,000 of ordinary income. Put another way, once the losses soak up all the capital gains, up to $3,000 of the losses may be used to absorb up to $3,000 of ordinary income. Here, because the total losses,  {fm.ac(total_losses)}, do not exceed the total gains, {fm.ac(total_gains)}, plus $3,000, there were no losses left to carry forward. More specifically: {answer_lang} Consider <a href="https://www.law.cornell.edu/uscode/text/26/1211" target="_new" rel="noreferrer">Section 1211(b)</a>.</p>',usable_loss:f'That is the correct amount of loss that can be used in {fm.current_year}. But the question asks how much loss can be carried forward.',random_answer:'This answer was randomly generated.'}
                    
                if correct > 0:
                    
                    judgements = {correct:f'<p>That is correct! {person1.name} is an individual, so the losses that may be used in {fm.current_year} equal the total amount of capital gains, {fm.ac(total_gains)}, plus up to $3,000 of ordinary income. Here, because the total losses, {fm.ac(total_losses)}, exceed the total gains, {fm.ac(total_gains)}, plus $3,000 by {fm.ac(loss_carryforward)}, that amount can be carried forward. More specifically: {answer_lang} Consider also <a href="https://www.law.cornell.edu/uscode/text/26/1211" target="_new" rel="noreferrer">Section 1211(b)</a>.</p>',loss_carryforward_soak_up_all_OI:f'<p>What about the limitations in <a href="https://www.law.cornell.edu/uscode/text/26/1211" target="_new" rel="noreferrer">Section 1211(b)(1)</a>?</p>',loss_carryforward_no_3000:f'<p>What about the $3000 referred to in <a href="https://www.law.cornell.edu/uscode/text/26/1211" target="_new" rel="noreferrer">Section 1211(b)(1)</a>?</p>',0:f'{person1.name} is an individual, so the losses that may be used in {fm.current_year} equal the total amount of capital gains, {fm.ac(total_gains)}, plus up to $3,000. But the total losses are greater than {fm.ac(total_gains+3000)}. What happens to the rest of the losses?', random_answer:'This answer was randomly generated.'}
        ...
        while len(possibleanswers) < 5:
                (possibleanswers, judgements) = fm.random_answer_pot(possibleanswers,judgements,3,start=60,end=110)
...
def section_1231_netting():
    ...
    asset_purchase_lang = ""
    answer_lang = ""
    loss_OI = gain_OI = loss_1231 = gain_1231 = net_all_gains_losses = net_all_gains_losses = 0
    ...
    if net_1231 < 0:
        ...
        if net_OI != 0:
            ...
            judgements = {correct:f"{correct_lang} Additionally, there is a net of {fm.ac(net_OI)} from assets that were held for less than one year and thus are not Section 1231 assets. This is a total of {fm.ac(total_OI)} treated as ordinary income. More specifically: {answer_lang}",random_answer:'This number was randomly generated.'}
    
        else:
        
            judgements = {correct:f"{correct_lang}. There is no other source of ordinary income or loss from the disposition of these assets. More specifically: {answer_lang}",random_answer:'This number was randomly generated.'}
    
    else:
        
        if net_OI != 0:

            judgements = {correct:f'{correct_lang} Additionally, there is a net of {fm.ac(net_OI)} from assets that were held for less than one year and thus are not Section 1231 assets. This is a total of {fm.ac(total_OI)} treated as ordinary. More specifically: {answer_lang}',random_answer:'This number was randomly generated.'}
    ...
    if net_OI != capital_all_gains_losses:
        ...
        judgements[capital_all_gains_losses] ='<p>You treated all gains and losses as capital. What about <a href="https://www.law.cornell.edu/uscode/text/26/1231" target="_new" rel="noreferrer">Section 1231(a)(2)</a>?</p>'
        
    if ordinary_all_gains_losses not in [total_OI,capital_all_gains_losses]:
        
        judgements[ordinary_all_gains_losses]='<p>You treated all gains and losses as ordinary. What about <a href="https://www.law.cornell.edu/uscode/text/26/1231" target="_new" rel="noreferrer">Section 1231(a)(1)</a>?</p>'
...
def recapture():
    ...
    total_depreciation = int(fm.depreciate_asset(asset,years_passed,initial_basis,sold=True)[0])
    basis_with_depreciation = initial_basis - total_depreciation

    possiblelevels = ['below basis with depreciation','above original basis','between original basis and basis with depreciation']
    ...
    if level_of_sale == 'below basis with depreciation':
        ...
        while True:        
            sale_price = fm.nearestthousand(random.randint(basis_with_depreciation,initial_basis))
            if sale_price not in [basis_with_depreciation,initial_basis] and basis_with_depreciation < sale_price < initial_basis:
                break
    ...
    if question_lang == possible_questions[0]:
        ...
        possibleanswers = ['Gain, some of which is ordinary, and some of which is 1231 gain, which will be characterized based on other 1231 transactions.','Gain, all of which is ordinary.','Gain, all of which is 1231 gain, which will be characterized based on other 1231 transactions.','Gain, some of which is ordinary, and some of which is capital.','Gain, all of which is capital.','Loss, some of which is ordinary, and some of which is 1231 loss, which will be characterized based on other 1231 transactions.','Loss, all of which is ordinary.','Loss, all of which is 1231 loss, which will be characterized based on other 1231 transactions.','Loss, some of which is ordinary, and some of which is capital.','Loss, all of which is capital.']
        
        if level_of_sale == 'below basis with depreciation':
            
            correct = 'Loss, all of which is 1231 loss, which will be characterized based on other 1231 transactions.'
            
            judgements_for_gain = {'Gain, some of which is ordinary, and some of which is 1231 gain, which will be characterized based on other 1231 transactions.':'This sale generates loss. Consider depreciation.','Gain, all of which is ordinary.':'This sale generates loss. Consider depreciation.','Gain, all of which is 1231 gain, which will be characterized based on other 1231 transactions.':'This sale generates loss. Consider depreciation.','Gain, some of which is ordinary, and some of which is capital.':'This sale generates loss. Consider depreciation.','Gain, all of which is capital.':'This sale generates loss. Consider depreciation.'}
            
            judgements_for_loss = {'Loss, some of which is ordinary, and some of which is 1231 loss, which will be characterized based on other 1231 transactions.':'You are correct that loss is generated. But why is some of the loss ordinary?','Loss, all of which is ordinary.':'You are correct that loss is generated. But why is the loss ordinary?','Loss, all of which is 1231 loss, which will be characterized based on other 1231 transactions.':f'Correct. Due to the total depreciation of {fm.ac(total_depreciation)}, the basis at sale was {fm.ac(basis_with_depreciation)}. Because the sale price was below the basis, the sale resulted in loss. Because the {asset.name} was used for business, was held for more than one year, and is depreciable, the {asset.name} is a quasi-capital asset, so any loss will be 1231 loss. We cannot know its character without knowing about {person1.poss} other 1231 gains and losses for the year.','Loss, some of which is ordinary, and some of which is capital.':f'<p>This is not a capital asset. Consider <a href="https://www.law.cornell.edu/uscode/text/26/1221" target="_new" rel="noreferrer">Section 1221(a)(2)</a>. How can you determine the character of the loss?</p>','Loss, all of which is capital.':f'<p>This is not a capital asset. Consider <a href="https://www.law.cornell.edu/uscode/text/26/1221" target="_new" rel="noreferrer">Section 1221(a)(2)</a>. How can you determine the character of the loss?</p>'}
            ...
            judgements_for_gain = {'Gain, some of which is ordinary, and some of which is 1231 gain, which will be characterized based on other 1231 transactions.':'Correct! Because the sale price is greater than the original basis, all of the depreciation will be recaptured. The gain due to the excess of the sale price over the original basis will be 1231 gain.','Gain, all of which is ordinary.':'Some of the gain, that due to the recapture of the depreciation, will definitely be ordinary. But how can you determine the character of the gain due to the excess of the sale price over the original basis?','Gain, all of which is 1231 gain, which will be characterized based on other 1231 transactions.':'Consider depreciation and recapture.','Gain, some of which is ordinary, and some of which is capital.':'Some of the gain, that due to the recapture of the depreciation, will definitely be ordinary. But how can you determine the character of the gain due to the excess of the sale price over the original basis.','Gain, all of which is capital.':'What about recapture? Also, how can you determine the character of the gain not due to recapture?'}
                                   
            judgements_for_loss = {'Loss, some of which is ordinary, and some of which is 1231 loss, which will be characterized based on other 1231 transactions.':'This sale generates gain, not loss. Double-check your depreciation calculations','Loss, all of which is ordinary.':'This sale generates gain, not loss. Double-check your depreciation calculations.','Loss, all of which is 1231 loss, which will be characterized based on other 1231 transactions.':'This sale generates gain, not loss. Double-check your depreciation calculations.','Loss, some of which is ordinary, and some of which is capital.':'This sale generates gain, not loss. Double-check your depreciation calculations.','Loss, all of which is capital.':'This sale generates gain, not loss. Double-check your depreciation calculations.'}
        
        elif level_of_sale == 'between original basis and basis with depreciation':
            
            correct = 'Gain, all of which is ordinary.'
            
            judgements_for_gain = {'Gain, some of which is ordinary, and some of which is 1231 gain, which will be characterized based on other 1231 transactions.':'The gain due to recapture is ordinary. Is there other gain?','Gain, all of which is ordinary.':f'<p>Correct! All of the gain is due to depreciation, so all of the gain is recharacterized as ordinary due to <a href="https://www.law.cornell.edu/uscode/text/26/1245" target="_new" rel="noreferrer">Section 1245</a> recapture.</p>','Gain, all of which is 1231 gain, which will be characterized based on all <a href="https://www.law.cornell.edu/uscode/text/26/1231" target="_new" rel="noreferrer">Section 1231</a> transactions for the year.':'Consider depreciation and recapture.','Gain, some of which is ordinary, and some of which is capital.':'The gain due to recapture is ordinary. Is there other gain?','Gain, all of which is capital.':'What about recapture?'}
            
            judgements_for_loss = {'Loss, some of which is ordinary, and some of which is 1231 loss, which will be characterized based on other 1231 transactions.':'This sale generates gain, not loss. Double-check your depreciation calculations','Loss, all of which is ordinary.':'This sale generates gain, not loss. Double-check your depreciation calculations.','Loss, all of which is 1231 loss, which will be characterized based on all Section 1231 transactions for the year.':'This sale generates gain, not loss. Double-check your depreciation calculations.','Loss, some of which is ordinary, and some of which is capital.':'This sale generates gain, not loss. Double-check your depreciation calculations.','Loss, all of which is capital.':'This sale generates gain, not loss. Double-check your depreciation calculations.'}
    ...
    if  question_lang == question_lang_ordinary_income:
        ...
        if level_of_sale == 'below basis with depreciation':
            ...
            judgements = {correct:f'<p>That is correct. Because the total depreciation, {fm.ac(total_depreciation)}, results in a basis, {fm.ac(basis_with_depreciation)}, that exceeds the sale price, the disposition results in loss. <a href="https://www.law.cornell.edu/uscode/text/26/1245" target="_new" rel="noreferrer">Section 1245</a> does not result in recapture when there is a loss on the sale. The loss is <a href="https://www.law.cornell.edu/uscode/text/26/1231" target="_new" rel="noreferrer">Section 1231</a> loss, and therefore its character is based on all 1231 transactions for the year.</p>',gainloss:f'<p>This is the loss generated by the sale. But <a href="https://www.law.cornell.edu/uscode/text/26/1245" target="_new" rel="noreferrer">Section 1245</a> does not result in recapture when there is a loss on the sale. Additionally, the loss is Section 1231 loss, and therefore its character is based on all <a href="https://www.law.cornell.edu/uscode/text/26/1231" target="_new" rel="noreferrer">Section 1231</a> transactions for the year. We thus cannot know its character without additional information.</p>',total_depreciation:'This is the total depreciation for the asset. The sale of this asset results in a loss. Why would the total depreciation be recaptured under <a href="https://www.law.cornell.edu/uscode/text/26/1245" target="_new" rel="noreferrer">Section 1245</a>?',random_answer:'This number was randomly generated.'  }
            
        elif level_of_sale == 'above original basis':
            
            correct = recapture
            
            judgements = {recapture:f'<p>Correct! The total depreciation, {fm.ac(total_depreciation)}, results in a basis of {fm.ac(basis_with_depreciation)}. The portion of this gain that is due to recapture, {fm.ac(recapture)}, is ordinary income under <a href="https://www.law.cornell.edu/uscode/text/26/1245" target="_new" rel="noreferrer">Section 1245</a>. Because this is a <a href="https://www.law.cornell.edu/uscode/text/26/1231" target="_new" rel="noreferrer">Section 1231</a> asset, the remainder of the gain is 1231 gain, and its character will be determined by taking into account all 1231 gain and loss for the year.</p>',0:f'<p>It is true that this is a <a href="https://www.law.cornell.edu/uscode/text/26/1231" target="_new" rel="noreferrer">Section 1231</a> asset, so to the extent that the regular 1231 rules apply, we cannot know whether this gain is ordinary or capital (because that will be determined based on all 1231 transactions for the year). But what about <a href="https://www.law.cornell.edu/uscode/text/26/1245" target="_new" rel="noreferrer">Section 1245</a>?</p>',gainloss:'This is the full amount of gain. It is true that some of this is ordinary. But do we know that all of it is ordinary? What type of asset is this asset, given that it is a depreciable asset used in a trade or business and held for more than one year?',random_answer:'This number was randomly generated.'}
            
        elif level_of_sale == 'between original basis and basis with depreciation':
            
            correct = recapture
                
            judgements = {recapture:f'<p>Correct! The total depreciation, {fm.ac(total_depreciation)}, results in a basis of {fm.ac(basis_with_depreciation)}. Because the sale price is between the original basis and the basis after depreciation, all {fm.ac(recapture)} of the gain is due to depreciation, so all is recaptured as ordinary income under <a href="https://www.law.cornell.edu/uscode/text/26/1245" target="_new" rel="noreferrer">Section 1245</a>.</p>',0:f'<p>It is true that this is a <a href="https://www.law.cornell.edu/uscode/text/26/1231" target="_new" rel="noreferrer">Section 1231</a> asset, so to the extent that the regular 1231 rules apply, we cannot know whether this gain is ordinary or capital (because that will be determined based on all 1231 transactions for the year). But what about <a href="https://www.law.cornell.edu/uscode/text/26/1245" target="_new" rel="noreferrer">Section 1245</a>?</p>',random_answer:'This number was randomly generated.',total_depreciation:'This is the amount of depreciation. How do you calculate the basis of the property at sale, given this amount of depreciation?'}
...
def asset_sale_all():
    ...
    for item in assets_for_problem:
        asset_purchase_lang = asset_purchase_lang + f"<br>{item.problem_facts} {item.period_lang}<br>"
    ...
    offsetting = min(net_CL,ordinary,3000)
    ...
    #     correct = ordinary_total
        ...
        if net_CL == 0:
            ...
            summary_answer_lang_addl = f"There is an excess of {fm.ac(net_CL)} of capital losses over gains. Before using any offsetting capital losses, there is {fm.ac(ordinary)} of ordinary income. Capital losses can be used to offset {fm.ac(offsetting)} of that, which reduces the capital losses accordingly. Therefore {fm.ac(carryforward)} of losses are carried into the next year.<br>"
...
def section_24_credit():
    ...
    for i in range(0,number_of_children):        
        ...
        individual_lang = f" {child_name}, {taxpayer.name}'s {relationship}{individual_age_lang}"
    ...
    if marital_status == 'married':
        pronouns = [f'{taxpayer.name} is married to {spouse.name}, and they file a joint tax return. They have','they','their','provide',f'{taxpayer.name} and {spouse.name}']
        threshold = fm.section_24_threshold.marriedlimit
    else:
        pronouns = [f'{taxpayer.name} is unmarried and files as head of household. {taxpayer.name} has' ,f'{taxpayer.nom}',f'{taxpayer.poss}','provides',f'{taxpayer.name}']
    ...
    if phaseout_type == 'phaseout full':
        ...
        while True:
            magi = int(fm.nearesthundred(random.randint(threshold,(20*credit_amt - 1000 + threshold))))
    ...
    possibleanswers = [final_credit,0,credit_amt,final_credit_no_fraction,random_answer,true_phaseout]
    
    if phaseout_type == 'phaseout full':
    
        judgements={correct:f'This is correct. The number of qualifying children is {number_qual_child}. The number of dependents who are not qualifying children for purposes of the child tax credit is {number_qual_rel}. This would initially lead to a credit of {number_qual_child} x {fm.ac(full_credit)} + {number_qual_rel} x $500 = {fm.ac(credit_amt)}. However, adjusted gross income exceeds the threshold amount, {fm.ac(threshold)}, by more than {int(fractions_of_thousand_for_phaseout):,} thousands or fractions thereof, so there is sufficient AGI to phase out the entire credit.',credit_amt:f'<p>What about the phaseout in <a href="https://www.law.cornell.edu/uscode/text/26/24" target="_new" rel="noreferrer">24(b)</a>, as modified in Section 24(h)(3)?</p>',random_answer:'This number was randomly generated.'}
        
        if final_credit_no_fraction != 0:
            
            judgements[final_credit_no_fraction] = f'<p>What is the significance of the language "or fraction thereof" in <a href="https://www.law.cornell.edu/uscode/text/26/24" target="_new" rel="noreferrer">24(b)</a>?</p>'
    
    elif phaseout_type == 'phaseout partial':
        
        judgements={correct:f'This is correct. The number of qualifying children is {number_qual_child}. The number of dependents who are not qualifying children for purposes of the child tax credit is {number_qual_rel}. This would initially lead to a credit of {number_qual_child} x {fm.ac(full_credit)} + {number_qual_rel} x $500 = {fm.ac(credit_amt)}. However, adjusted gross income exceeds the threshold amount, {fm.ac(threshold)}, by {int(math.ceil((magi-threshold)/1000)):,} thousands or fractions thereof, so the credit is reduced by $50 x {int(math.ceil((magi-threshold)/1000)):,}, for a total of {fm.ac(credit_amt)} - {fm.ac(true_phaseout)} = {fm.ac(final_credit)}.',credit_amt:f'<p>What about the phaseout in <a href="https://www.law.cornell.edu/uscode/text/26/24" target="_new" rel="noreferrer">24(b)</a>, as modified in Section 24(h)(3)?</p>',random_answer:'This number was randomly generated.',0:'Is the credit entirely phased out?'}
        ...
        if true_phaseout != final_credit:
            ...
            judgements[true_phaseout]='This is the correct phaseout. But the question is: how much credit is left after the phaseout?'
    
    elif phaseout_type == 'no phaseout':
        
        judgements={correct:f'This is correct. The number of qualifying children is {number_qual_child}. The number of dependents who are not qualifying children for purposes of the child tax credit is {number_qual_rel}. The adjusted gross income is not above the threshold of {fm.ac(threshold)}, so the total credit is {number_qual_child} x {fm.ac(full_credit)} + {number_qual_rel} x $500 = {fm.ac(credit_amt)}, with no phaseout.',random_answer:'This number was randomly generated.',0:'Is the credit phased out?'}
...
def section_21_credit():
    ...
    if marital_status == 'married':
        pronouns = [f'{taxpayer.name} is married to {spouse.name}, and they file a joint tax return. They have','they','their','provide','live']
        threshold = fm.section_24_threshold.marriedlimit
    else:
        pronouns = [f'{taxpayer.name} is unmarried and files as head of household. {taxpayer.name} has' ,f'{taxpayer.nom}',f'{taxpayer.poss}','provides','lives']
    ...
    for i in range(0,number_of_children):        
        ...
        individual_age_lang=f', who is {age_child}'
        if marital_status == 'married':
            individual_lang = f" {child_name}, {pronouns[2]} {relationship}"+individual_age_lang
        else:
            individual_lang = f" {child_name}, {taxpayer.name}'s {relationship}"+individual_age_lang
    ...
    DCAP_choices = [[f"{taxpayer.name} elects the maximum amount of contribution to {taxpayer.poss} employer's dependent care assistance program ('DCAP') under Section 129.",5000],["",0]]
    
    if number_qual_child > 0:
        amt_spent = int(fm.generate_random_item(max_creditable_amt,70,130)+random.randint(1,9)*100)
   ...
   #phaseout, no 'fraction thereof'
    ...
    final_credit_no_fraction = int(creditable_expenses * max(20,app_percent_init-no_fraction_thereof_phaseout)/100)
    ...
    pretend_q_final_credit_no_phaseout = int(pretend_q_creditable_expenses * app_percent_init/100)
    
    pretend_q_final_credit = int(pretend_q_creditable_expenses * app_percent/100)
    pretend_q_final_credit_no_limit_phaseout = int(pretend_q_creditable_expenses * no_limit_phaseout/100)
    pretend_q_final_credit_no_fraction = int(pretend_q_creditable_expenses * max(20,app_percent_init-no_fraction_thereof_phaseout)/100)
    ...
    if number_of_children == 1 and age_child == old:
       ...
       possibleanswers = possibleanswers + [pretend_q_final_credit_no_phaseout,pretend_q_final_credit_no_fraction]
     
       judgements={correct:f'<p>Correct. {taxpayer.name} has no qualifying individuals under <a href="https://www.law.cornell.edu/uscode/text/26/21" target="_new" rel="noreferrer">21(b)(1)(A)</a>, because {child_name} is older than 12.</p>',pretend_q_final_credit:f'<p>How old is {child_name}? Consider <a href="https://www.law.cornell.edu/uscode/text/26/21" target="_new" rel="noreferrer">21(b)(1)(A)</a></p>',pretend_q_final_credit_no_phaseout:f'<p>How old is {child_name}? Consider <a href="https://www.law.cornell.edu/uscode/text/26/21" target="_new" rel="noreferrer">21(b)(1)(A)</a>.</p>',pretend_q_final_credit_no_fraction:f'<p>How old is {child_name}? Consider <a href="https://www.law.cornell.edu/uscode/text/26/21" target="_new" rel="noreferrer">21(b)(1)(A)</a></p>',amt_spent:f'This is the amount that {taxpayer.name} spent, but the question is what credit is available due to those expenses.',pretend_q_final_credit_no_limit_phaseout:f'<p>How old is {child_name}? Consider <a href="https://www.law.cornell.edu/uscode/text/26/21" target="_new" rel="noreferrer">21(b)(1)(A)</a>.</p>'}

    else:

        possibleanswers = possibleanswers +[final_credit,max_creditable_amt]
        
        judgements={amt_spent:f'This is the amount that {taxpayer.name} spent, but the question is what credit is available due to those expenses.',max_creditable_amt:'<p>This is the maximum amount of employment expenses eligible for the credit, but is the full amount of these expenses creditable? Consider <a href="https://www.law.cornell.edu/uscode/text/26/21" target="_new" rel="noreferrer">21(a)(1)</a>.</p>'}
        
        if DCAP_lang != "":
                
             possibleanswers.append(final_credit_no_DCAP)
                
             judgements[final_credit_no_DCAP]='<p>What about the interaction of the DCAP and Section 21 described in <a href="https://www.law.cornell.edu/uscode/text/26/21" target="_new" rel="noreferrer">21(c)</a>?</p>'
        
        if phaseout_type == 'no phaseout':
                
            judgements[correct]=f'This is correct. The number of qualifying children is {number_qual_child}, which means that the maximum amount of employment-related expenses eligible for the credit is {fm.ac(max_creditable_amt)}. {taxpayer.name} spent {fm.ac(amt_spent)}, so the creditable amount is {fm.ac(creditable_expenses_no_DCAP)}. {DCAP_answer_lang} The credit percentage is not phased out, because AGI is not greater than $15,000. Therefore the final credit is {app_percent} percent times  {fm.ac(creditable_expenses)} = {fm.ac(final_credit)}.'
            ...
            judgements[final_credit_no_phaseout]=f'<p>What about the phaseout in <a href="https://www.law.cornell.edu/uscode/text/26/21" target="_new" rel="noreferrer">21(a)(2)</a>?</p>'
            judgements[final_credit_no_fraction]=f'<p>What is the significance of the language "or fraction thereof" in <a href="https://www.law.cornell.edu/uscode/text/26/21" target="_new" rel="noreferrer">21(a)(2)</a>?</p>'
            
            if phaseout_type == 'phaseout full':
                
                possibleanswers.append(final_credit_no_limit_phaseout)
            
                judgements[correct]=f'This is correct. The number of qualifying individuals is {number_qual_child}, which means that the maximum amount of employment-related expenses eligible for the credit is {fm.ac(max_creditable_amt)}. {taxpayer.name} spent {fm.ac(amt_spent)}, so the creditable amount is {fm.ac(creditable_expenses_no_DCAP)}. {DCAP_answer_lang} The credit percentage is phased down to 20 percent, because AGI is greater than $43,000. Therefore the final credit is 20 percent x {fm.ac(creditable_expenses)} = {fm.ac(final_credit)}.'

                judgements[final_credit_no_limit_phaseout]=f'<p>What is the significance of the parenthetical "(but not below 20 percent)" in <a href="https://www.law.cornell.edu/uscode/text/26/21" target="_new" rel="noreferrer">21(a)(2)</a>?</p>'
...
functions_dict = {'rates':rates_problems,'time value of money':tvm,'unrestricted property as compensation':unrestricted_property,'restricted property as compensation':restricted_property,'options as compensation':options_as_comp,'different ways of satisfying a debt':satisfy_debt,'bonds, shifting interest rates, and COD':bonds_COD,'gain from sale of principal residence':principal_res,'COD exclusion':exclusion_COD,'qualified employee discount':qual_empl_disc,'charitable donation deduction':charitable_donation,'home mortgage interest deduction':qri,'taxable property exchange':property_exchange,'basis':basis_problems,'like-kind exchanges':like_kind,'transactions with liabilities':liabilities,'installment sales':installment_sales,'depreciation':depreciation_question,'capital gains and losses':cap_gain_netting,'section 1231 netting':section_1231_netting,'recapture':recapture,'section 24 credit':section_24_credit,'section 21 credit':section_21_credit,'capital gain + section 1231 + recapture':asset_sale_all}
