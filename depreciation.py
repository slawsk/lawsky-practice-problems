...
class CapGainsFacts:
        def __init__(self,problem_lang,answer_lang,LTCG,LTCL,STCG,STCL,ordinary,carryforward,long_term_carryforward,short_term_carryforward,total_losses,total_gains,offset,offset_from_short_term,offset_from_long_term):
...
full_asset_list = [computer,copier,helicopter,car,taxi,bus,light_truck,heavy_truck,locomotive,tractor,trailer,barge,furniture]

class AssetFacts:
    def __init__(self,asset, date_purchased,purchase_price,date_sold,sale_price,long_or_short,gain_or_loss_no_depreciation,problem_facts,period_lang,answer_facts_cap_gain,answer_facts_1231,answer_facts_all_netting,depreciated_basis,recapture,amount_1231,ordinary,type_of_gain):
        self.asset = asset
...
def create_asset_facts(type='asset',year=fm.current_year):
    ...
    if long_or_short == 'long':
        date_purchased = fm.date_before(date_sold,soonest=366,latest=(asset.recovery_period-1)*365)
        type_of_gain = '1231'
    ...
    if type == 'stock':
        ...
        [total_depreciation,annual_depreciation] = fm.depreciate_asset(asset,years_depreciation,purchase_price,True)
    ...
    assetforlist = AssetFacts(asset, date_purchased,purchase_price,date_sold,sale_price,long_or_short,gain_or_loss,problem_facts,period_lang,answer_facts_cap_gain,answer_facts_1231,answer_facts_all_netting,depreciated_basis,recapture,amount_1231,ordinary,type_of_gain)
...
def create_cap_gain_year(year=fm.current_year):
    ...
    # while True:
        ...
        ordinary_income_lang = f"<br><br>They also have {fm.ac(ordinary)} of ordinary income in {year}."
        ...
        offset_from_long_term = offset_from_short_term = long_term_carryforward = short_term_carryforward = 0
        problem_lang = f"{asset_purchase_lang} {ordinary_income_lang}"
        
        # if carryforward > 0:
        #     break
    
        year_facts = CapGainsFacts(problem_lang,answer_lang,LTCG,LTCL,STCG,STCL,ordinary,carryforward,long_term_carryforward,short_term_carryforward,total_losses,total_gains,offset,offset_from_short_term,offset_from_long_term)   
